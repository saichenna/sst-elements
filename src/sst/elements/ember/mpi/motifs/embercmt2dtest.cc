        ...
        sendy_pos(false), sendy_neg(false)
{
	...
	equationsfile = "test2.txt";

	//Check to enable gaussian distribution for the compute times
        isstochastic = (uint32_t) params.find("arg.dostochastic", false);;
...
void EmberCMT2DGenerator::configure()
{
	...
	embercompute = new Embercompute(equationsfile,myID);
...
bool EmberCMT2DGenerator::generate( std::queue<EmberEvent*>& evQ)
{
	...
	if (docompute) {
		...
		if (isstochastic) {
			...
			nsCompute = outputvaluesdist.find("computens")->second->getNextDouble();
			...
			nsCompute = outputvalues.find("comptuens")->second;
			enQ_compute(evQ,nsCompute);
		}
	}
...
void EmberCMT2DGenerator::generate_computetimes(){


        //1. Generate the hashmap/dictionary of input parameter values that are used in the compute model parameters
        ...
        modelparams.insert(std::pair<std::string,uint32_t>("procFlops",procFlops));
        modelparams.insert(std::pair<std::string,uint32_t>("procFreq",procFreq));



        //2. Generate the compute times for all the compute blocks in the motifs based on the analytical models provided in the equations file
        outputvalues = embercompute->staticcompute(modelparams);
...
void EmberCMT3DGenerator::generate_distributions(){
        ...
        outputvaluesdist = embercompute->stochasticcompute();
